...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($usuario_id) && !empty($fecha_hora_entrada) && !empty($fecha_hora_salida)) {
        ...
        if ($stmt->execute()) {
            header("Location: ../vista/finingreso_salida.php?success=1");
            exit();
        } else {
            header("Location: ../vista/finingreso_salida.php?error=1");
            exit();
        }

        $stmt->close();
    } else {
        header("Location: ../vista/finingreso_salida.php?error=2");
        exit();
    }
}
?>
